...
const Tasks: React.FC<TasksProps> = ({ user }) => {
  ...
  const [forceUpdate, setForceUpdate] = useState(0); // To trigger re-renders on localStorage changes

  useEffect(() => {
    loadTasks();
    // Simulate Real-time "Job Drop"
  ...
  const loadTasks = async () => {
    ...
    setTasks(data.filter(t => t.status === TaskStatus.OPEN));
    setLoading(false);
  };
  ...
  const handleAttemptApply = (task: Task, e?: React.MouseEvent) => {
    ...
    // Feature Gate: Check if user is active
    if (!user.is_active && user.role !== 'admin') {
      navigate('/activate');
      return;
    }

    if (taskService.hasUserBid(task.id, user.id)) {
        alert("You have already applied for this task.");
        return;
    }

    setTaskToApply(task);
    setSelectedTask(null); // Close detail view if open
  };
  ...
  const handleConfirmApplication = async () => {
    ...
    if (result.success) {
      ...
      setForceUpdate(prev => prev + 1); // Refresh bid counts ui
    } else {
  ...
  const handleAdminViewBids = (task: Task, e?: React.MouseEvent) => {
      if (e) e.stopPropagation();
      const bids = taskService.getBids(task.id);
      setCurrentBids(bids);
  ...
  const handleAdminAssign = async (bid: Bid) => {
      ...
      if (res.success) {
          ...
          loadTasks(); // Refresh list to remove assigned task
      } else {
                ...
                <div className="p-4 rounded-xl bg-slate-50 border border-slate-100 flex items-center gap-3">
                   ...
                   <div>
                     ...
                     <p className="text-lg font-bold text-slate-900">
                       {taskService.getBidCount(selectedTask.id)} <span className="text-sm font-normal text-slate-500">Bids</span>
                     </p>
                   </div>
                </div>
              </div>
            ...
            <div className="p-6 bg-slate-50 border-t border-slate-100 flex justify-end gap-3 sticky bottom-0">
              ...
              {user.role === UserRole.ADMIN ? (
                ...
                <button
                    ...
                    disabled={taskService.hasUserBid(selectedTask.id, user.id)}
                    className={`px-8 py-2.5 font-bold text-sm rounded-lg transition-all shadow-lg flex items-center gap-2 ${
                    user.is_active 
                        ? (taskService.hasUserBid(selectedTask.id, user.id) ? 'bg-slate-300 text-slate-500 cursor-not-allowed' : 'bg-slate-900 text-white hover:bg-blue-600 shadow-slate-900/20')
                        : 'bg-green-600 text-white hover:bg-green-700 shadow-green-500/20'
                    }`}
                >
                    {user.is_active 
                        ? (taskService.hasUserBid(selectedTask.id, user.id) ? 'Applied' : 'Apply Now') 
      ...
      {loading ? (
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {tasks
            ...
            .map(task => {
                const hasApplied = taskService.hasUserBid(task.id, user.id);
                const applicantCount = taskService.getBidCount(task.id);
